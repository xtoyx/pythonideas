...
def find_next_page():
    for i in range(0,2):
        ...
        print('finished until page :', i*10+1)
        time.sleep(30)
...
def make_adir():
    dir = os.path.join(f'{os.getcwd()}','pages_jobpost')
    if not os.path.exists(dir):
        os.mkdir(dir)
    dir = os.path.join(f'{os.getcwd()}\pages_jobpost',f'pages-jobpost-{subject}')
    if not os.path.exists(dir):
        os.mkdir(dir)
...
def write_to_file():
    ...
    for index in range(0,int(len(skills_important)/10)):
        with open(f'pages_jobpost/pages-jobpost-{subject}/pickledump-untilpage{(index+1)*1}.txt', "w")as r:
            r.write('')
            r.close
                ...
                if index < len(skills_important):
                    if unfamilar_skill not in skills_important[index].lower():
                        ...
                        "\n more_info": more_infos[index],
                            ...
                            #dump
                        with open(f'pages_jobpost/pages-jobpost-{subject}/pickledump-untilpage{(howmanytimes+1)*1}.txt', "ab") as fp:
                            pickle.dump(data, fp)
            fp.close()
